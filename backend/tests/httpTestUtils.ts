...
export function startTestServer(): TestServer { const authStore = new AuthStore(); const profileStore = new ProfileStore(); const friendStore = new FriendStore(profileStore); const server: http.Server = createApp(undefined, undefined, profileStore, friendStore, undefined, undefined, authStore); let base = ''; const ready = new Promise<void>((resolve) => server.listen(0, () => { base = `http://localhost:${(server.address() as AddressInfo).port}`; resolve(); })); return { baseUrl: () => base, close: () => new Promise((resolve) => server.close(() => resolve())), authStore, profileStore, friendStore, ready }; }
