...
export function startTestServer(options: ApiServerOptions = {}): TestServer { const authStore = new AuthStore(); const profileStore = new ProfileStore(); const friendStore = new FriendStore(profileStore); const effectiveOptions: ApiServerOptions = process.env.DATABASE_URL || options.socialActivityStore ? options : { ...options, socialActivityStore: { touch: async () => undefined, getFriendActivity: async () => [] } }; const server: http.Server = createApp(undefined, undefined, profileStore, friendStore, undefined, undefined, authStore, undefined, undefined, undefined, effectiveOptions); let base = ''; const ready = new Promise<void>((resolve) => server.listen(0, () => { base = `http://localhost:${(server.address() as AddressInfo).port}`; resolve(); })); return { baseUrl: () => base, close: () => new Promise((resolve) => server.close(() => resolve())), authStore, profileStore, friendStore, ready }; }
