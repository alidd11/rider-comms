...
describe('profiles, social links, friends, and messages', { skip: !hasDatabase && 'DATABASE_URL not set; skipping Postgres-backed social tests' }, () => {
  ...
  it('resolves a whole ride roster in one batched profile request, silently dropping unknown and blocked riders', async () => {
    ctx.authStore.createTestSession('batch-viewer');
    ctx.authStore.createTestSession('batch-alice');
    ctx.authStore.createTestSession('batch-blocked');
    await ctx.profileStore.update('batch-alice', { instagramUsername: 'ali.rides', instagramVisibility: 'public' });
    assert.equal((await postJson(ctx, 'batch-blocked', '/blocks', { riderId: 'batch-viewer' })).status, 200);

    const res = await postJson(ctx, 'batch-viewer', '/profiles/batch', {
      riderIds: ['batch-alice', 'batch-blocked', 'batch-unknown-rider', 'batch-viewer'],
    });
    assert.equal(res.status, 200);
    const { profiles } = await res.json() as { profiles: Record<string, { instagramUsername: string }> };
    assert.deepEqual(Object.keys(profiles), ['batch-alice']);
    assert.equal(profiles['batch-alice']?.instagramUsername, 'ali.rides');
  });
  it('rejects an empty or oversized batch profile request', async () => {
    ctx.authStore.createTestSession('batch-bounds-viewer');
    assert.equal((await postJson(ctx, 'batch-bounds-viewer', '/profiles/batch', { riderIds: [] })).status, 400);
    assert.equal((await postJson(ctx, 'batch-bounds-viewer', '/profiles/batch', {
      riderIds: Array.from({ length: 51 }, (_, i) => `rider-${i}`),
    })).status, 400);
  });
