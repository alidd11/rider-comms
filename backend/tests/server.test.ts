...
// (see db.ts) — that one test needs DATABASE_URL pointing at a reachable
// Postgres instance and is skipped otherwise, rather than failing every run
// in a sandbox with no database.
...
describe('authenticated API', () => {
  ...
  it('deletes an account and revokes its token', { skip: !hasDatabase && 'DATABASE_URL not set; skipping Postgres-backed account deletion test' }, async () => { const session = ctx.authStore.createTestSession('delete-me'); const headers = { Authorization: `Bearer ${session.token}` }; assert.equal((await fetch(`${ctx.baseUrl()}/auth/me`, { method: 'DELETE', headers })).status, 200); assert.equal((await fetch(`${ctx.baseUrl()}/auth/me`, { headers })).status, 401); assert.equal(ctx.authStore.hasRider('delete-me'), false); });
