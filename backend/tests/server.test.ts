...
describe('authenticated API', () => {
  ...
  it('does not expose the retired guest identity minting endpoint', async () => {
    const unauthenticated = await fetch(`${ctx.baseUrl()}/auth/guest`, { method: 'POST' });
    assert.equal(unauthenticated.status, 401);
    const token = ctx.authStore.createTestSession('existing-account').token;
    const authenticated = await fetch(`${ctx.baseUrl()}/auth/guest`, {
      method: 'POST',
