...
const publicFiles = [
  'index.html', 'app.css', 'app.js', 'avatar-system.js', 'navigation-road-events.js', 'navigation-guidance.js', 'position-interpolation.js', 'movement-safety.js', 'message-state.js', 'config.js',
...
const required = ['index.html', 'app.css', 'app.js', 'avatar-system.js', 'navigation-road-events.js', 'navigation-guidance.js', 'position-interpolation.js', 'movement-safety.js', 'message-state.js', 'config.js', 'manifest.json', 'sw.js'];
...
const [rawCss, rawJs, rawAvatarSystem, rawNavigationRoadEvents, rawNavigationGuidance, rawPositionInterpolation, rawMovementSafety, rawMessageState] = await Promise.all([
...
const version = createHash('sha256').update(rawCss).update(rawJs).update(rawAvatarSystem).update(rawNavigationRoadEvents).update(rawNavigationGuidance).update(rawPositionInterpolation).update(rawMovementSafety).update(rawMessageState).digest('hex').slice(0, 10);
...
for (const asset of ['app.css', 'config.js', 'avatar-system.js', 'navigation-road-events.js', 'navigation-guidance.js', 'position-interpolation.js', 'app.js', 'movement-safety.js', 'message-state.js', 'manifest.json', 'routes.css', 'routes.js']) {
