...
describe('Settings mockup contract', () => {
  ...
  it('keeps mockup card and row geometry compact', () => {
    assert.match(settingsSource, /profileCard:\s*\{[\s\S]*?minHeight:\s*78[\s\S]*?marginBottom:\s*8[\s\S]*?borderRadius:\s*8/);
    ...
    assert.match(settingsSource, /<RiderAvatar avatarId=\{avatarId\} size=\{56\} \/>/);
    ...
    assert.match(settingsSource, /secondarySettingsGroup:\s*\{\s*marginTop:\s*10\s*\}/);
    assert.match(settingsSource, /signOutGroup:\s*\{\s*marginTop:\s*10\s*\}/);
  });
